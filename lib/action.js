...
export const addNews = async(prevState, FormData) => {
    const {headline, creator, description, email} = Object.fromEntries(FormData)
    try {
        connectDB()
        const newNews = new News({
            headline, creator, description, email
        })
...
export const updateNews = async (FormData) => {
    const { id, headline, creator, description, email } = Object.fromEntries(FormData)
    try {
        connectDB();
        const updateFields = { headline, creator, description, email }
